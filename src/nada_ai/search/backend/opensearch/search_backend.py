...
def _normalize_facets(aggregations: dict[str, Any] | None, facet_fields: list[str]) -> dict[str, list[dict[str, Any]]]:
    if not aggregations or not facet_fields:
        return {}
...
class OpenSearchSearchBackend:
    ...
    async def search(self, params: SearchParams) -> SearchOutcome:
        ...
        if params.use_idno_fast_path:
            ...
            if facet_fields:
                merge_facets_into_body(q, facet_fields)

        resp = await self._client.search(index=self._settings.index_name, body=q)
        total_val, hits_out = _hits_from_response(resp)
        facets = _normalize_facets(resp.get("aggregations"), facet_fields or []) if facet_fields else None
